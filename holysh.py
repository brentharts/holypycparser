...
def run(co, filename=None, symbol=None):
    global HOLYC
    # wrapping text with `` sends text directly to cmd
    if co.startswith("`") and co.endswith("`"):
        ...
        os.chdir(co[len("Cd") + 1 : -2])
    elif co.startswith("Print"):
        print(co[len("Print") + 1 : -2])
    elif co.startswith("Ed"):
        os.system("nano " + co[len("nano ") + 1 : -2])
    else:
        try:
            # __runsource(co)
            exec(co, scope, scope)
            ...
            if co in os.listdir("/usr/bin/"):
                print(
                    '"%s" is in /usr/bin/ you can run it with backticks `%s`' % (co, co)
                )
    return False
